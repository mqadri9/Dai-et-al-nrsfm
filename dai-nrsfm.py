...
def retrieve_rotation(Mhat, gk, numFrames):
    ...
    for i in range(2, numFrames+1):
        ...
        if np.linalg.det(R_i) < 0:
            R_i = -R_i
        theta = math.acos((np.trace((R_i.T).dot(R_tmp))-1)/2)*180/np.pi
        if theta > 90:
            R_i[0:2] = -R_i[0:2]
        R[2*i-2:2*i] = R_i[0:2]
        R_tmp = R_i
...
def recover_S(W, R, S0, K):
    ...
    for i in range(1,20):
        ...
        while(non_converged):
            ...
            residuals = np.linalg.norm(S_sharp - S_sharp_k_prev, ord='fro')/max(1, np.linalg.norm(S_sharp_k_prev, ord='fro'))
